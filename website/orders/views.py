...
def addresssubmit(request):
    if request.method == "POST":
        if request.POST['address-submit']:
            ...
            p=Address(user_id=user_id,fullname=fullname,mobile=mobile,pincode=pincode,town=town,area=area)
            p.save()
        return redirect('orders:order_create')
...
# def check_dist(addid):    
#     address=Address.objects.get(id=addid)
#     new_add=addresses.street + "," + address.town
#     nom=Nominatim()
#     n=nom.geocode(new_add)
#     lat=n.latitude
#     lng=n.longitude
#     print(n.latitude)
#     print(n.longitude)
